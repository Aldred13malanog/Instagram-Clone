...
export function viewCommentSection(button, i) {
	...
	if (info.typePost === 'image') {
		post = `
			 <img src="${info.post}" data-id="${info.id}">
		...
		post = `
			<video autoplay loop src="${info.post}" data-id="${info.id}"></video>
			<div class="image-like js-cimage-like">
				<svg aria-label="Unlike" fill="currentColor" role="img" viewBox="0 0 48 48"><title>Unlike</title><path d="M34.6 3.1c-4.5 0-7.9 1.8-10.6 5.6-2.7-3.7-6.1-5.5-10.6-5.5C6 3.1 0 9.6 0 17.6c0 7.3 5.4 12 10.6 16.5.6.5 1.3 1.1 1.9 1.7l2.3 2c4.4 3.9 6.6 5.9 7.6 6.5.5.3 1.1.5 1.6.5s1.1-.2 1.6-.5c1-.6 2.8-2.2 7.8-6.8l2-1.8c.7-.6 1.3-1.2 2-1.7C42.7 29.6 48 25 48 17.6c0-8-6-14.5-13.4-14.5z"></path></svg>
			</div>
		...
		<div class="comment-right-section">
			...
			<div class="comment-icon-con">
				<div class="comment-icon">
					<button class="js-clike-button" data-id="${info.id}">
				...
				<span class="comment-post-like js-clike-counts">${info.likeCount.toLocaleString()} likes</span>
				<span class="comment-time">${info.timePosted}</span>
			</div>
	...
	function showComments(info) {
		...
		commentsData.forEach(comments => {
			if (info.id === comments.id) {
				matchingData = comments;
			}
		});
	...
	document.querySelector('.js-ccomment-button').addEventListener('click', () => {
		document.querySelector('.js-cinput').focus();
	});

	document.querySelector('.js-cshare-button').addEventListener('click', () => {
	...
	document.querySelector('.js-cmore-option-button').addEventListener('click', () => {
